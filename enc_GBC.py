...
class GranularBall(nn.Module):
    ...
    def forward(self, x, tau=1.0):
        ...
        return self.refine(out), att, sigma, dif, self.centers
...
class GBC_S_EncDec(nn.Module):
    ...
    def forward(self, x, return_details=True):
        ...
        t3 = out
        # enc_feature= t3
        t3_gbc, att_t3, sigma, dif_t3, enc_balls_centers = self.gbc(t3)
        ...
        dec_feature = out
        out, att_out, _, dif_out, dec_ball_centers = self.gbc(out)
        ...
        if return_details:
            return out, t3, t3_gbc, att_t3, dec_feature, sigma, enc_balls_centers
